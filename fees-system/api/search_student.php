...
=======================================================-->
<?php
// Start session to access $_SESSION['inst_id']
...
// Access secure session variables
...
if (!empty($term) && !empty($instId)) {
    // Step 1: Get Student and Course ID (Filtered for Active Students & Institute)
    ...
    $stmt->bind_param("ssi", $term, $term, $instId); // Added 'i' for inst_id
    $stmt->execute();
    ...
    if ($student) {
        ...
        // Logic for Miscellaneous fallback
        $foundMisc = false;
...
// Ensure JSON output even if student is not found
