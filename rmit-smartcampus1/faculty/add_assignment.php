...
/* FETCH COURSES */
$cq = $conn->prepare("
  SELECT id, course_name, program, semester
  FROM staff_courses
  WHERE staff_id=?
  ORDER BY created_at DESC
");
$cq->bind_param("i",$staffId);
$cq->execute();
$courses = $cq->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
      ...
      VALUES (?,?,?,?)
    ");
    $stmt->bind_param("isss",$courseId,$title,$dueDate,$desc);
    $stmt->execute();

    $conn->query("INSERT INTO staff_activity (staff_id,activity)
                  VALUES ($staffId,'Added assignment: $title')");
...
<head>
  ...
  <!-- Page Title -->
  <title>RMIT SmartCampus Portal | Faculty Dashboard | Add Assignment |<?= htmlspecialchars($instituteCd) ?></title>
